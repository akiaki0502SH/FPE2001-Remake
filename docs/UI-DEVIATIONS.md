...
本文件记录 FPE2001-Remake 实现与 `FPE2001-Remake UI/UX 实施规格 v2.9`（Deep Boundary Blue + Native DWM Frame + 程序名聚合 + 自动扫描/字节级编辑）之间的结构差异（验收项：UIUX 规格 §17）。
...
## v2.9 扫描类型自动判断 + 扫描结果直接操作 + 编辑器字节级定位 — 2026-08-16

### 实施记录

**扫描（M01）**

- 新增「自动」扫描类型并设为默认：首次扫描内存只读一遍，同时按 8/16/32 位整数三宽度判定（每宽度独立 FPSN 快照）；再次扫描各快照独立过滤，自动收敛出真实类型与地址 —— 解决模拟器游戏（SFC 等）8/16 位存储数值用 32 位搜索不到的问题。
- 候选列表新增「类型」列（8 位/16 位/32 位/64 位）；首次/再次扫描后状态栏显示各宽度命中分布（如 `8 位 1,120,000 / 16 位 2,900 / 32 位 300`），0 候选时给出诊断提示（编码/类型、数值又变化、进程实例选错）。
- 值输入支持 `0x` 十六进制（如 `0x21`=33），输入后按 Enter 直接扫描（扫描页与编辑器查找框均支持）。
- 扫描性能：消除热路径全部分配（ValueComparer 循环外预编译期望值 + span 化、ProcessHandle `unsafe fixed` 零拷贝 P/Invoke、FpsnSnapshot 256 KiB 写缓冲 + 块读迭代、宽度特化热循环、进度 200 ms 节流）——64 MiB 全内存自动扫描由 11.9 s 降至 0.23 s（约 51 倍），再次扫描 44 万候选过滤约 0.3 s。

**扫描结果直接操作（原版 FPE2001 能力）**

- 「写入」：弹输入框改值 → 自动建立地址表条目 → 立即写内存（写前比较 + 读回校验），候选行当前值同步刷新。
- 「锁定」：持续把选中候选值写回内存（Always，250 ms，失败熔断），再点一次解除；条目同步进入地址表。
- 「在编辑器中打开」：跳转十六进制编辑器并定位到该地址。

**十六进制编辑器（M03）**

- 字节级选中：十六进制区拆为 16 个独立字节列（列头 00–0F）+ `SelectionUnit=Cell`，点击单个字节高亮（黄色 `#FFD54F` + 深色字），编辑框自动预填该字节 hex。
- 打开即定位：绝对地址 → 视图内偏移映射修复（4KB 窗口内越界导致定位失效）；目标行滚到可视区第一行、目标字节列滚到最左列（左上角第一位）并高亮。
- 进程内存编辑「应用覆盖」即写回目标进程（无需手动保存）；写前比较失败（游戏值已变化）自动刷新视图提示重试。
- 修复 `SelectionUnit=Cell` 下 `ScrollIntoView` 选择整行抛 `InvalidOperationException` 导致的闪退（先清空选中再滚动）；新增全局异常兜底（crash 日志写 `%TEMP%\FPE2001-Remake\logs\`）。

### 验证记录

- 探针实证（真实 snes9x，32 位进程、793 个可读区域 404 MB）：自动扫描 21 → 改写候选为 19 → 再次扫描收敛出 3 条（同地址 8/16/32 位视图）；编辑器字节写入读回一致；定位链路行/字节列/预填值全命中。
- 集成测试：22 通过（新增 `AutoScan_21_to_19_Converges_On_UInt8`、`AutoScan_RegionTail_KeepsCandidate` 回归）。
- 单元测试：80 通过（新增 `VirtualHexRows` 缓存一致性回归，修复定位引用不一致）。
- Release 构建：0 警告、0 错误。
- 测试程序：`artifacts/ui-process-groups-v39/Fpe2001Remake.UI.exe`。
- 对应文档：UI/UX v2.9、代码实施规格 v2.6、Win11 架构设计 v2.6。

## v3.0 Design Kit v2.1 视觉实现 — 2026-08-17

### 实施边界

- 本轮只替换 WPF 表现层：主题令牌、控件样式、窗口壳、模块导航、动作栏、页面标题区、设置页和关于页。
- 保留 `MainViewModel`、全部模块 ViewModel、Command、扫描引擎、地址表、冻结服务、文件扫描、十六进制编辑器和适配器通信契约。
- 九模块顺序与原有快捷键保持不变；窗口按钮仍由 `SystemCommands` 处理，DWM 原生边框仍由 `Windows11WindowFrame` 处理。

### 视觉实现

- 主题切换为 Design Kit v2.1 的低饱和灰青令牌：`#D9DDDF` 画布、`#F5F6F6` 窗口、`#526F75` 主色、`#CFD5D6` 分隔线。
- 窗口结构固定为标题栏 44 DIP、九模块导航 66 DIP、模块动作栏 44 DIP、工作区和状态栏 31 DIP。
- 模块导航使用九等分横向布局，图标与名称两行居中，隐藏内部开发标记，活动模块以白色边框和轻阴影表达。
- 页面统一增加标题/说明区；面板采用 1 DIP 边界、9 DIP 圆角和轻量卡片阴影；设置页改为左分类 + 右表单；新增完整关于页。

### 验证记录

- UI Release 编译：0 警告、0 错误。
- 单元测试：80 通过、0 失败、0 跳过。
- 集成测试：22 通过、0 失败、0 跳过。
- 发布程序：`artifacts/ui-design-kit-v30/Fpe2001Remake.UI.exe`。
- 用户实机重点回归：Snes9x 目标附加、自动类型扫描、21→19 再次扫描、写入/冻结、地址表、十六进制编辑器和窗口按钮。
- 对应基准：`docs/design-kit-v2.1/screens/` 与 `docs/specifications/FPE2001-Remake_UIUX实施规格_v3.0.docx`。
